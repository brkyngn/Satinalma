...
import { getPersonnel, updatePersonnel } from "@/lib/services/personnel";
...
export default async function PersonelDuzenlePage({
  ...
  async function kaydet(formData: FormData) {
    ...
    if (!parsed.success) {
      redirect(`/demirbas/tanimlar/personel/${id}?error=${encodeURIComponent(parsed.error.issues[0].message)}`);
    }

    await updatePersonnel(id, parsed.data);
    redirect("/demirbas/tanimlar/personel");
  }

  return (
    <div className="max-w-lg">
      <h2 className="mb-4 text-base font-semibold text-zinc-900">Personel Düzenle</h2>
      <form action={kaydet} className="space-y-4 rounded-lg border border-zinc-200 bg-white p-6">
        <div>
          ...
          <input name="fullName" required defaultValue={person.fullName} className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm" />
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div>
            <label className="mb-1 block text-sm font-medium text-zinc-700">Ünvan</label>
            <input name="title" defaultValue={person.title ?? ""} className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm" />
          </div>
          <div>
            <label className="mb-1 block text-sm font-medium text-zinc-700">Departman</label>
            <input name="department" defaultValue={person.department ?? ""} className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm" />
          </div>
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium text-zinc-700">Telefon</label>
          <input name="phone" defaultValue={person.phone ?? ""} className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm" />
        </div>
        <label className="flex items-center gap-2 text-sm text-zinc-700">
          <input type="checkbox" name="active" defaultChecked={person.active} className="rounded border-zinc-300" />
          Aktif
        </label>
