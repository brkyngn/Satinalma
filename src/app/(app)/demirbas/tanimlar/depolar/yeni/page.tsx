...
import { createWarehouse } from "@/lib/services/warehouses";
import { SubmitButton } from "@/components/SubmitButton";

export default async function YeniDepoPage({
  searchParams,
}: {
  ...
  async function ekle(formData: FormData) {
    ...
    if (!parsed.success) {
      redirect(`/demirbas/tanimlar/depolar/yeni?error=${encodeURIComponent(parsed.error.issues[0].message)}`);
    }
  ...
  return (
    <div className="max-w-lg">
      <h2 className="mb-4 text-base font-semibold text-zinc-900">Yeni Depo / Konum</h2>
      <form action={ekle} className="space-y-4 rounded-lg border border-zinc-200 bg-white p-6">
        <div>
          ...
          <input name="name" required className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm" />
        </div>
        <div>
          <label className="mb-1 block text-sm font-medium text-zinc-700">Tip</label>
          <select name="type" defaultValue="merkez" className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm">
        ...
        <div>
          ...
          <input name="address" className="w-full rounded-md border border-zinc-300 px-3 py-2 text-sm" />
        </div>
